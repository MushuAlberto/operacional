...
def get_img_as_base64(file_path):
    """Lee una imagen local y la convierte a string Base64 para incrustar en HTML"""
    try:
...
if file_tablero:
    try:
        ...
        with col_img_izq:
            if os.path.exists("logoSQM-li-90.png"):
                st.image("logoSQM-li-90.png", width=120)
            else:
                st.warning("Falta logoSQM")
            
        with col_img_der:
            if os.path.exists("Image20240314124309.png"):
        ...
        for idx, prod in enumerate(productos_ordenados):
            with tabs[idx]:
                ...
                df_destinos = df_p.groupby('Destino').agg({
                    'Ton_Prog': 'sum', 'Ton_Real': 'sum', 'Eq_Prog': 'sum', 'Eq_Real': 'sum', 'Fecha': 'count'
                ...
                st.dataframe(df_destinos_display, use_container_width=True, hide_index=True, height=300)
        ...
        if generar_html:
            with st.spinner("Generando reporte HTML interactivo..."):
                ...
                # --- PREPARAR IMAGEN PARA HTML (Base64) ---
                logo_sqm_b64 = get_img_as_base64("logoSQM-li-90.png")
                # Si no encuentra el logo local, usamos uno público como respaldo o vacío
                img_tag = f'<img src="{logo_sqm_b64}" style="max-height: 80px;">' if logo_sqm_b64 else '<div style="color:#2E7D32; font-weight:bold;">SQM</div>'
...
<head>
    ...
    <style>
        body {{
            ...
            margin: 0; padding: 20px; background: #f4f6f9;
        }}
        .container {{
            max-width: 1400px; margin: 0 auto; background: white; border-radius: 10px; padding: 40px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);
        }}
        .header {{
            text-align: center; margin-bottom: 40px; border-bottom: 3px solid #2E7D32; padding-bottom: 20px;
        }}
        .header h1 {{ color: #2E7D32; font-size: 2.2em; margin: 0; }}
        .kpi-container {{
            display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin: 30px 0;
        }}
        .kpi-card {{
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 10px; text-align: center;
        }}
        .kpi-value {{ font-size: 2em; font-weight: bold; margin: 10px 0; }}
        .section {{ margin: 40px 0; }}
        .section-title {{
            font-size: 1.5em; color: #333; border-left: 5px solid #2E7D32; padding-left: 15px; margin-bottom: 20px;
        }}
        .tabs {{ display: flex; gap: 10px; margin-bottom: 20px; flex-wrap: wrap; }}
        .tab {{
            padding: 10px 20px; background: #f0f0f0; border: none; border-radius: 5px; cursor: pointer; transition: 0.3s;
        }}
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="kpi-container">
                <div class="kpi-card"><div class="kpi-label">Tonelaje Total</div><div class="kpi-value">{total_ton_real:,.0f}</div></div>
                <div class="kpi-card"><div class="kpi-label">Equipos Totales</div><div class="kpi-value">{total_eq_real:.0f}</div></div>
                <div class="kpi-card"><div class="kpi-label">Cumplimiento</div><div class="kpi-value">{cumplimiento_general:.1f}%</div></div>
                <div class="kpi-card"><div class="kpi-label">Productos</div><div class="kpi-value">{num_productos}</div></div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                <div id="grafico_toneladas" style="flex: 1; min-width: 400px; height: 400px;"></div>
                <div id="grafico_cumplimiento" style="flex: 1; min-width: 400px; height: 400px;"></div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <table>
                <thead>
                    <tr><th>Producto</th><th>Ton. Prog</th><th>Ton. Real</th><th>Diferencia</th><th>Cumplimiento</th></tr>
                </thead>
                <tbody>
"""
...
"""
                for idx, prod in enumerate(productos_ordenados):
                    ...
                    html_content += f'<button class="tab {active_class}" onclick="openTab(event, \'tab{idx}\')">{prod}</button>\n'
                
                html_content += "</div>"
                
                for idx, prod in enumerate(productos_ordenados):
                    style_display = "block" if idx == 0 else "none"
                    html_content += f"""
                    <div id="tab{idx}" class="tab-content" style="display: {style_display};">
                        <h3>{prod}</h3>
...
"""
                # --- INYECCIÓN DE DATOS DE LOS GRÁFICOS ---
                
                # 1. GRÁFICOS GENERALES (Aquí estaba el fallo de los gráficos invisibles)
...
"""
                ...
                for idx, prod in enumerate(productos_ordenados):
                    ...
                    json_prod = fig_prod.to_json()
                    html_content += f"""
        var data_prod_{idx} = {json_prod};
...
"""
        ...
        if generar_correo:
            with st.spinner("Generando texto de correo..."):
                # (Lógica original de correo se mantiene intacta)
                productos_alerta = []
                productos_ok = []
...
"""
                if productos_alerta:
                    correo_texto += f"""<div class="alert"><strong>⚠️ Productos que Requieren Atención:</strong><br>{', '.join(productos_alerta)}</div>"""
                if productos_ok:
                    correo_texto += f"""<div class="success"><strong>✅ Productos con Cumplimiento Alcanzado:</strong><br>{', '.join(productos_ok)}</div>"""
                
                correo_texto += """<h3>Ranking de Productos</h3><table><thead><tr><th>Producto</th><th>Ton. Real</th><th>Cumplimiento</th></tr></thead><tbody>"""
                for _, row in df_ranking.head(5).iterrows():
                    correo_texto += f"""<tr><td>{row['Producto']}</td><td>{row['Ton_Real']:,.0f}</td><td>{row['Cumplimiento']:.1f}%</td></tr>"""
                correo_texto += f"""</tbody></table><div class="footer"><p>Reporte generado el {datetime.now().strftime('%d/%m/%Y a las %H:%M')}</p></div></body></html>"""
                ...
                st.download_button("Descargar HTML del Correo", correo_texto, f"correo_reporte_{fecha_sel.strftime('%Y%m%d')}.html", "text/html", use_container_width=True)
